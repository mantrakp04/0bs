...
export default defineSchema({
  ...
  checkpointBlobs: defineTable({
    threadId: v.string(),
    checkpointNs: v.string(),
    channel: v.string(),
    version: v.string(),
    type: v.string(),
    blob: v.optional(v.array(v.number())),
  })
    .index("by_thread_ns_channel", ["threadId", "checkpointNs", "channel"])
    .index("by_thread_ns_channel_version", [
      "threadId",
      "checkpointNs",
      "channel",
      "version",
  ...
  checkpointWrites: defineTable({
    ...
    channel: v.string(),
    type: v.string(),
    blob: v.array(v.number()),
  })
    .index("by_thread_ns_checkpoint", [
      "threadId",
      "checkpointNs",
      "checkpointId",
    ])
    .index("by_thread_ns_checkpoint_task", [
      "threadId",
      "checkpointNs",
      "checkpointId",
      "taskId",
    ])
    .index("by_thread_ns_checkpoint_task_idx", [
      "threadId",
      "checkpointNs",
      "checkpointId",
      "taskId",
